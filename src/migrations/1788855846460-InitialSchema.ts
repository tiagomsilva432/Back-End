...
export class InitialSchema1788855846460 implements MigrationInterface {
    name = 'InitialSchema1788855846460'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE "salaries" ("id" uuid NOT NULL DEFAULT uuidv7(), "user_id" uuid NOT NULL, "amount" numeric(12,2) NOT NULL, "currency" character varying(3) NOT NULL DEFAULT 'EUR', "effective_from" date NOT NULL, "effective_to" date, "reason" character varying(255), "created_by" uuid NOT NULL, "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), CONSTRAINT "ck_salaries_period_valid" CHECK ("effective_to" IS NULL OR "effective_to" > "effective_from"), CONSTRAINT "ck_salaries_amount_non_negative" CHECK ("amount" >= 0), CONSTRAINT "PK_20ca60aa8d4201c7bcb430fdb36" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE UNIQUE INDEX "uq_salary_current" ON "salaries" ("user_id") WHERE "effective_to" IS NULL`);
        await queryRunner.query(`CREATE TABLE "projects" ("id" uuid NOT NULL DEFAULT uuidv7(), "company_id" uuid NOT NULL, "manager_id" uuid, "name" character varying(150) NOT NULL, "client_name" character varying(150), "status" character varying(20) NOT NULL DEFAULT 'planned', "start_date" date, "end_date" date, "budget" numeric(14,2), "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), CONSTRAINT "uq_projects_company_name" UNIQUE ("company_id", "name"), CONSTRAINT "ck_projects_dates_valid" CHECK ("end_date" IS NULL OR "start_date" IS NULL OR "end_date" >= "start_date"), CONSTRAINT "PK_6271df0a7aed1d6c0691ce6ac50" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "allocations" ("id" uuid NOT NULL DEFAULT uuidv7(), "user_id" uuid NOT NULL, "project_id" uuid NOT NULL, "allocation_percent" smallint NOT NULL, "role_on_project" character varying(100), "start_date" date NOT NULL DEFAULT ('now'::text)::date, "end_date" date, "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), CONSTRAINT "ck_allocations_dates_valid" CHECK ("end_date" IS NULL OR "end_date" >= "start_date"), CONSTRAINT "ck_allocations_percent_range" CHECK ("allocation_percent" BETWEEN 1 AND 100), CONSTRAINT "PK_ca63099fc248466264af0fa6f1f" PRIMARY KEY ("id"))`);
        ...
        await queryRunner.query(`CREATE TABLE "skills" ("id" uuid NOT NULL DEFAULT uuidv7(), "name" character varying(100) NOT NULL, "category" character varying(20) NOT NULL DEFAULT 'other', CONSTRAINT "uq_skills_name" UNIQUE ("name"), CONSTRAINT "PK_0d3212120f4ecedf90864d7e298" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "employee_skills" ("user_id" uuid NOT NULL, "skill_id" uuid NOT NULL, "proficiency" smallint, "years_experience" numeric(4,1), CONSTRAINT "ck_employee_skills_years_non_negative" CHECK ("years_experience" >= 0), CONSTRAINT "ck_employee_skills_proficiency_range" CHECK ("proficiency" BETWEEN 1 AND 5), CONSTRAINT "PK_899425f40ff845ee1ac9cb6c75e" PRIMARY KEY ("user_id", "skill_id"))`);
        await queryRunner.query(`CREATE TABLE "users" ("id" uuid NOT NULL DEFAULT uuidv7(), "company_id" uuid NOT NULL, "email" character varying(255) NOT NULL, "password_hash" character varying(255), "signup_token" uuid, "signup_token_expires_at" TIMESTAMP WITH TIME ZONE, "role" character varying(20) NOT NULL DEFAULT 'employee', "status" character varying(20) NOT NULL DEFAULT 'invited', "must_change_password" boolean NOT NULL DEFAULT true, "email_verified_at" TIMESTAMP WITH TIME ZONE, "last_login_at" TIMESTAMP WITH TIME ZONE, "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "updated_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "deleted_at" TIMESTAMP WITH TIME ZONE, CONSTRAINT "uq_users_signup_token" UNIQUE ("signup_token"), CONSTRAINT "uq_users_company_email" UNIQUE ("company_id", "email"), CONSTRAINT "PK_a3ffb1c0c8416b9fc6f907b7433" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "companies" ("id" uuid NOT NULL DEFAULT uuidv7(), "name" character varying(150) NOT NULL, "tax_id" character varying(50), "country" character varying(2) NOT NULL DEFAULT 'PT', "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), CONSTRAINT "uq_companies_tax_id" UNIQUE ("tax_id"), CONSTRAINT "PK_d4bc3e82a314fa9e29f652c2c22" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "reviews" ("id" uuid NOT NULL DEFAULT uuidv7(), "request_id" uuid NOT NULL, "recommends_raise" boolean NOT NULL, "performance_score" smallint, "comments" text, "submitted_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), CONSTRAINT "REL_6ea6d2328e7d32aa0b0970d8d9" UNIQUE ("request_id"), CONSTRAINT "ck_reviews_score_range" CHECK ("performance_score" BETWEEN 1 AND 5), CONSTRAINT "PK_231ae565c273ee700b283f15c1d" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "review_requests" ("id" uuid NOT NULL DEFAULT uuidv7(), "company_id" uuid NOT NULL, "cycle_id" uuid, "requested_by" uuid NOT NULL, "reviewer_id" uuid NOT NULL, "reviewee_id" uuid NOT NULL, "status" character varying(20) NOT NULL DEFAULT 'pending', "due_date" date, "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), CONSTRAINT "ck_review_requests_distinct_parties" CHECK ("reviewer_id" <> "reviewee_id"), CONSTRAINT "PK_01e5bec2adcef1cf498e861f75a" PRIMARY KEY ("id"))`);
        ...
        await queryRunner.query(`CREATE TABLE "review_cycles" ("id" uuid NOT NULL DEFAULT uuidv7(), "company_id" uuid NOT NULL, "name" character varying(100) NOT NULL, "opened_by" uuid NOT NULL, "deadline" date, "closed_at" TIMESTAMP WITH TIME ZONE, "created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), CONSTRAINT "uq_review_cycles_company_name" UNIQUE ("company_id", "name"), CONSTRAINT "PK_5634972955eaa909a8ff55736a7" PRIMARY KEY ("id"))`);
